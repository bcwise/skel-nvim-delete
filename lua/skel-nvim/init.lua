...
-- Handle updating the "Last modified" time of the skeleton files.
vim.api.nvim_create_autocmd({ "BufWritePre", "FileWritePre" }, {
	pattern = { "*.c", "*.cpp", "*.h" },
	callback = function()
		M.update_last_modified_timestamp()
	end,
	group = skel_autogroup,
})

function M.update_last_modified_timestamp()
	...
	if vim.fn.line("$") >= 7 then
		...
		local l = 1
		while l <= 7 do
			vim.fn.setline(l, vim.fn.substitute(vim.fn.getline(l), "Last modified: \\zs.*", time, "gc"))
			l = l + 1
		end
		if vim.fn.search("\\vLast (modified|change|update):?", "wncp") > 0 then
